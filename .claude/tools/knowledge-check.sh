#!/usr/bin/env bash
# Knowledge Integrity v1: is a knowledge-base record structurally valid, and do the concrete
...
# syntax. This base deliberately cites nine other repositories and scratch repositories, so a bare
# backticked path or a bare hex string is not evidence about this tree, and guessing which is which
# from prose would be noise. Measured 2026-09-05: of 469 backticked spans containing a slash, 60
# resolved here and 409 did not, and 20 of 50 backticked hex strings name commits in other trees.
# So bare forms are left alone, and the explicit `repo:` and `commit:` forms are checked.
#
# SPEED: two awk passes per page, and every existence test is a shell builtin. The first build ran
# a subprocess per field per page and took 67 seconds on Windows, which is long enough that someone
# would start skipping it. Keep it that way: no per-field pipelines.
...
# its own shape in section 11 of the rules, one line per open item grouped by who can act, and the
# base's README is its front door. Both are named in the rules and neither is a durable knowledge
# page. Nothing else is exempt, and a new page is in scope the moment it is written. Both are still
# subject to every reference check below: being exempt from the header exempts nothing else.
...
if [ ! -f "$conf" ]; then
  ...
  while IFS= read -r line; do
    ...
    # "[a-z]+ acceptance tests" also matches "the acceptance tests", and reading "the" as the stated
