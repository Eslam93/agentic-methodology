...
### D-19 · Tier 3 finishes on a completed independent review or an explicit owner waiver · 2026-09-05
**Decision:** a Tier 3 task is complete only when one independent review has finished, or the owner has waived it in their own words. The review menu is unchanged and the owner still picks the route; what changes is that choosing nothing is no longer a silent path. The two independent routes are the fresh-context review and the Codex cold pass; the local run with `/test-guide` is not one, because the builder runs it. `baseline.sh seal` now takes the tier and records it in the brief's front matter; `baseline.sh review <task> completed <route> <result>` or `review <task> waived "<the owner's words>"` records which happened, once, with a timestamp; `baseline.sh check`, which `/work` already runs at hand-back, exits non-zero on a Tier 3 brief with neither. A review that was offered, chosen, started, or that failed is none of these: `completed` refuses to record unless a result is given, and the route must be one of the two. What no check can do is tell whether the result is true or the words were said, which is where the contract stays instruction-level. **Options:** leave the menu free, as D-06 had it; make one route mandatory, which would take the choice from the owner; a Stop-hook gate, rejected because it would interrupt every turn of the build for a condition that only matters at the end. **Why:** Tier 3 is the tier where a wrong implementation costs the most, and the menu let a task finish with no independent judgment and no record that anyone decided to skip it. The value is the distinction a later reader can now make between "reviewed" and "not reviewed because the owner said so". **By:** owner, 2026-09-05. **Reversible:** yes; the fields are additive and the check is one branch. **Revisit when:** the waiver is used routinely rather than deliberately, which would mean the tier is set too high, or when a review route the kit treats as independent is added. **Amends:** D-06, which keeps its menu and its defaults; only the silent-absence path is removed. Evidence: `_investigations/2026-09-05-tier3-review/methodology.md`.

### D-18 · A session-scoped active-task pointer; both hooks read it instead of guessing · 2026-09-05
**Decision:** `baseline.sh seal` writes `working/active-tasks/<session id>`, one line holding `working/<task>/brief.md`, in the same step that seals the brief; the session id is the host's own, read from `CLAUDE_CODE_SESSION_ID`. `resume-brief` restores the brief that pointer names, and `verify-on-finish` takes its baseline from the same brief, so neither infers a task from file recency. The pointer resolves only to `working/<one segment>/brief.md`, which excludes a relay brief, an absolute path, and a traversal; the session id is validated before it is used as a file name. When there is no valid pointer, resume falls back to the newest task brief one level deep, never a relay brief, and says it is a guess, while the Stop hook falls back to `HEAD`, the behaviour before the baseline existed, and never picks among sealed briefs. `closed_at` and `baseline.sh close` are removed: the pointer decides which baseline is live, and a builder cannot retire its own baseline before acceptance. **Options:** a single `working/active-task` file, which two sessions on one checkout would overwrite; a pending-file handshake promoted by a new hook, which was unnecessary once the environment variable was measured; keeping `closed_at` beside the pointer, which would leave two mechanisms answering one question. **Why:** the compaction hook restored `/codex-relay`'s own brief and called it the owner's agreement, because the relay writes after the agreement and the rule was newest-wins; the real defect was that nothing recorded which task a session was carrying. One identity, recorded once at the yes, serves resume and task-relative verification and removes both heuristics. **By:** owner, 2026-09-05. **Reversible:** yes; both hooks keep their pre-pointer fallbacks. **Revisit when:** the host stops setting `CLAUDE_CODE_SESSION_ID` for Bash tool calls, which `seal` reports out loud while both hooks drop to their fallbacks, or when a stale pointer from a finished session is shown to cause harm, or when subagents stop sharing their parent's session id. **Supersedes:** the newest-by-modification-time rule in `resume-brief`, and the open-seal scan and `closed_at` lifecycle introduced by D-17. Evidence: `_investigations/2026-09-05-active-task/methodology.md`.

### D-17 · The approved brief carries the task baseline; the Stop hook diffs tests against it · 2026-09-05
**Decision:** at the owner's yes, `baseline.sh seal` writes `approved_at`, one `baseline_commit.<checkout>` per checkout, and `brief_sha256` (the digest of the text below the front matter) into the front matter of `working/<task>/brief.md` itself, and lists the files already dirty in `pre-existing.txt` beside it; task commits stay on task-owned paths and leave those files alone. `verify-on-finish` read every brief carrying an unclosed seal, named the brief in each finding, and compared test files against that brief's commit with rename detection; a test added during the task was compared against `HEAD`; a sealed commit rewritten by a rebase from its merge-base; with no open seal the comparison was against `HEAD` as before; `baseline.sh close` added `closed_at` at acceptance. `baseline.sh check <task>` at hand-back reports a brief changed after approval. A sealed brief cannot be re-sealed: an approval baseline does not move, and a changed agreement becomes a new task brief beside the original. No waiver mechanism. **Amended the same day by D-18:** the scan, `close`, and `closed_at` are gone; a session-scoped pointer names the one brief whose baseline applies. Everything else here stands. **Options:** keep comparing against `HEAD` only, which shows nothing once a weakening is committed and misses a staged rename; a separate `working/<task>/baseline` state file, built first on 2026-09-05 and withdrawn the same day at the owner's objection that it was a parallel task-state system; a signed brief with a lock file; a per-task allow-list enforced by a hook. **Why:** five findings of the external review of 2026-09-05 shared one missing fact, the task's starting point, and the smallest record of it closes all five; keeping that record inside the agreed brief means one task has exactly one baseline, a check is told which task it is checking, and the values survive a compaction because `resume-brief` already prints the brief. The friction of a baseline that keeps blocking a legitimately weakened test is accepted until a real incident earns a waiver. **By:** owner, in the brief and the follow-up of 2026-09-05. **Reversible:** yes; remove the tool and the hooks' seal sections. **Revisit when:** a legitimate baseline-relative test change blocks a turn for real. **Supersedes:** the "commit a checkpoint" line of the standing orders, for a task that has a brief. Evidence: `_investigations/2026-09-05-task-baseline/methodology.md`.
...
**Decision:** rules `standing-orders`, `writing`, `working-here`, `knowledge-base` (path-scoped); skills `orient`, `work`, `codex-relay`, `test-guide`, `pr`, `board`, `record`, `handoff`, `explain`, `summarize`; hooks `guard-secrets`, `guard-commands`, `verify-on-finish`, `resume-brief`, each in PowerShell and Bash; tools `layout.sh`, `verify.sh`, `install`. Budget: one ratchet on always-loaded rule lines, set at 400 on 2026-09-05 when the kit's three generic rules measured 229 lines; raised only deliberately, with the reason recorded in `verify.sh`. Skills 200 lines, `/work` 250. **Options:** one large `/work`; plugin packaging; a separate budget for the kit's rules, tried on 2026-09-05 and dropped because project traps append to the traps rule and would fail it at the first entry. **Why:** a skill over 400 lines gets skimmed; the desktop app has no plugin surface. **By:** owner, with the budget figure set by the builder from the measurement. **Reversible:** yes. **Revisit when:** a skill goes unused for a quarter, then delete it. **Supersedes:** S-11.
