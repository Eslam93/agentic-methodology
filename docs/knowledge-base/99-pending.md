...
- `P1` `resume-brief` restores the newest `brief.md` by modification time and matches `working/relay/<task>/brief.md`, so a Codex relay run after the agreement replaces the task brief. Fix: an active-task pointer read first, and the relay folder excluded. The same pointer would tell the Stop hook which sealed brief is in flight and replace `closed_at` (D-17). Evidence: the same page, R2 and new finding 2.
