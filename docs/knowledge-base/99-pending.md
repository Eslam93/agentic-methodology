...
- `P2` The brief digest covers the body only, so the `tier:` line and the review record can be hand-edited after sealing and `check` will not notice: one edit turning `tier: 3` into `tier: 2` removes the Tier 3 contract. Reproduced 2026-09-05 by the verifier of D-19. The fix is to digest the sealed front-matter lines as well, which changes what `brief_sha256` means for every brief already sealed and so is a decision, not a patch. Evidence: `_investigations/2026-09-05-tier3-review/methodology.md`.
